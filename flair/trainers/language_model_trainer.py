...
def sentencepiece(seq):
    s = spm.SentencePieceProcessor()
    s.Load('./resources/piece_model/m.model')
    subwords_list = s.EncodeAsPieces(seq)
    subwords_list = [word.replace('▁', '') for word in subwords_list]
    return subwords_list

class TextCorpus(object):
    def __init__(self, path, dictionary: Dictionary, forward: bool = True, character_level: bool = True, subword: bool = False):
        
        self.subword = subword #add
        ...
        #add
        self.train_vocab = self.charsplit(os.path.join('/cl/work/shusuke-t/flair_myLM/resources/tasks/wmt11_sub', 'subwords_train'),
                                    expand_vocab=True,
                                    forward=False,
