...
class CharLMEmbeddings(TokenEmbeddings):
    ...
    def __init__(self, model, detach: bool = True):
        ...
        #import pdb; pdb.set_trace()
        dummy_sentence.add_token(Token('ICT_I_hello_me'))
    ...
    def _add_embeddings_internal(self, sentences: List[Sentence]) -> List[Sentence]:
        # get text sentences
        #import pdb; pdb.set_trace()
        undersplit_sentences = [sentence.to_tokenized_string().replace(' ', '_ _') for sentence in sentences] #fix
        text_sentences = [re.split('[_]', sentence) for sentence in undersplit_sentences] #fix
        ...
        for sentence_text in text_sentences:
            pad_by = longest_character_sequence_in_batch - len(sentence_text) #fix
            if self.is_forward_lm:
                padded = '\n_{}{}{}'.format('_'.join(sentence_text), end_marker, pad_by * '_ ')#fix
                append_padded_sentence(padded)
            else:
                padded = '\n_{}{}{}'.format('_'.join(sentence_text[::-1]), end_marker, pad_by * '_ ')#fix
                append_padded_sentence(padded)
        #import pdb; pdb.set_trace()
        ...
        for i, sentence in enumerate(sentences):
            ...
            offset_backward: int = len(re.split('[_ ]', sentence_text)) + extra_offset #fix
            for token in sentence.tokens:
                token: Token = token

                offset_forward += len(token.text.split('_')) #fix
                ...
                offset_backward -= len(token.text.split('_')) #fix

                token.set_embedding(self.name, embedding)

        return sentences

def sentencepiece_list_arg(seq_list):
    s = spm.SentencePieceProcessor()
    s.Load('/cl/work/shusuke-t/flair_myLM/resources/piece_model/m.model')
    subwords_list_list = [s.EncodeAsPieces(seq) for seq in seq_list]
    subwords_list_list = [[word.replace('▁', '') for word in subwords_list] for subwords_list in subwords_list_list]
    return subwords_list_list

def sentencepiece_list_return(seq):
    s = spm.SentencePieceProcessor()
    s.Load('/cl/work/shusuke-t/flair_myLM/resources/piece_model/m.model')
    subwords_list = s.EncodeAsPieces(seq)
    subwords_list = [word.replace('▁', '') for word in subwords_list]
    if subwords_list[0] == '_':
        subwords_list = subwords_list[1:]
